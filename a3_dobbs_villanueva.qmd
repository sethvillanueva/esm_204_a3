...
q_kink = nondac_demand_fn_q_from_p(50000)

d_agg_beyond_kink_fn <- function(Q){
  price = (demand_dac$coefficients[1] - (q_kink / (demand_dac$coefficients[2] + demand_nondac$coefficients[2]))) +
             (1/(demand_dac$coefficients[2] + demand_nondac$coefficients[2])) * Q
}
...
agg_demand_fn(10000)
nondac_demand_fn(10000)

agg_demand_fn(60000)
```

```{r}
demand_dac$coefficients[1] - (q_kink / (demand_dac$coefficients[2] + demand_nondac$coefficients[2]))
```
...
dac_demand_fn_q\_from_p \<- function(P){
